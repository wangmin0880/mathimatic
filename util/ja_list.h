 ...
 * @list.
 * 
 * 砍掉 head 之后， entry 之前的节点，放list 上。
 * Head 之后直接 接上entry.
 */
 ...
 *
 * 新的双链表粘接在 已有链表的head 和head->next 之间
 */
 ...
 *
 * 将大于一个节点的 list，除去第一个节点之后的所有节点， 插到head 和head->next 之间，
 * 同时初始化一下list这个首节点
 */
...
/**
 * 接下来的部分，有两种情况 带entry 和不带entry
 * list_for_each :这里的pos 和 head 都是list_head 指针
 * list_for_each_entry :这里的pos 是大结构体指针。head 是 list_head 指针
 */
